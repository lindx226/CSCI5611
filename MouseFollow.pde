...
//     Because circle() takes in the Width and Height of the ellipse, not the radius, so
//     the desired radius must be multiplied by 2.
...
void update(float dt){
  ...
  if(mousePos.minus(pos).length() > 5){
    vel = dir.times(dt * speed);
    pos.add(vel);
  }
}
